...
static void mps2_common_init(MachineState *machine)
{
    ...
    if (bios_name) {
        ...
        size = load_elf_as(filename, NULL, NULL, &entry, &lowaddr,
    ...
    mps2_binfo.kernel_cmdline = machine->kernel_cmdline;
    ...
    if (mps2_binfo.dtb_filename) {
        ...
        if (arm_load_dtb(mps2_binfo.dtb_start, &mps2_binfo, 0, as) <= 0) {
